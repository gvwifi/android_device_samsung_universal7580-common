...
/*******************************************************************
 * Camera2 wrapper fixup functions (Android 16 Post-VNDK refactor)
 *******************************************************************/
...
static char * camera2_fixup_getparams(int id __unused, const char * settings) {
    ...
    ALOGV("%s: Parameters: %s", __FUNCTION__, settings);
#endif
...
static int camera2_device_close(hw_device_t* device)
{
    ...
    std::lock_guard<std::mutex> lock(gCameraWrapperLock);
